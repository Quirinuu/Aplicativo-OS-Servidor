...
function mapPriority(p) {
  ...
  if (v.includes('urg') || v === '1')   return 'URGENT';
  if (v.includes('alta') || v === '2')  return 'HIGH';
  if (v.includes('baixa') || v === '4') return 'LOW';
  return 'MEDIUM';
}

// Grava o script em arquivo .ps1 temporário e executa com -File
// Isso evita que a senha !(&&!!)& quebre o parsing do -Command
function runPS1(scriptContent) {
  const tmpFile = path.join(os.tmpdir(), `sho_${Date.now()}.ps1`);
  try {
    // BOM UTF-8 garante que o PowerShell leia acentos corretamente
...
function buildConnScript(body) {
  // Usa here-string (@'...'@) para a connection string — imune a qualquer char especial
  return `
...
`;
  ...
  try {
    const output = runPS1(buildConnScript(body));
...
function discoverColumns() {
  ...
  // Tabelas que definitivamente NÃO são OS — ignora
  ...
  // Candidatos em ordem de prioridade — ORDEMS é o nome real no SHOficina
  const priority = [
    'ORDEMS', 'OS', 'OrdemServico', 'OrdensServico', 'ordem_servico',
    'Ordens_Servico', 'tblOS', 'tblOrdens', 'OSTable',
  ];

  // Junta: prioritários primeiro, depois o restante do banco (exceto blacklist)
  const candidates = [
    ...priority,
  ...
  for (const table of candidates) {
    ...
    if (rows && rows.length > 0) {
      ...
      // Valida que tem pelo menos uma coluna parecida com OS (cliente ou número)
      const looksLikeOS = cols.some(c => {
        const l = c.toLowerCase();
        return l.includes('cliente') || l.includes('numero') || l.includes('equipamento') ||
               l.includes('status') || l.includes('situac') || l.includes('defeito');
      });
      if (!looksLikeOS) {
        console.log(`⏭️  [SHOficina] Pulando "${table}" — não parece tabela de OS. Colunas: ${cols.join(', ')}`);
        continue;
      }
...
function inferColumns(columns) {
  ...
  return {
    id:           find('CODIGO'),
    osNumber:     find('CODIGO'),           // número da OS = CODIGO
    client:       find('COD_CLIENTE'),      // código — vai buscar nome na tabela CLIENTES
    equipment:    find('APARELHO'),         // nome do aparelho
    brand:        find('MARCA'),
    model:        find('MODELO'),
    serial:       find('SERIE'),
    ...
    defect:       find('DEFEITO'),
    observations: find('OBS_SERVICO'),      // observações do serviço (não do aparelho)
    status:       find('SITUACAO'),
    priority:     find('PRIOR'),
    createdAt:    find('ENTRADA'),          // data de entrada
    completedAt:  find('SAIDA'),            // data de saída
    pronto:       find('PRONTO'),           // flag de pronto (S/N)
  };
}
...
_poll() {
   ...
   _syncRow(row) {
    ...
    const osNumber   = extId; // CODIGO é tanto o ID quanto o número da OS

    // Nome do cliente vem do JOIN com CLIENTES
    ...
    const equipParts = [aparelho, marca, modelo].filter(Boolean);
    const equipment  = equipParts.join(' — ') || 'Equipamento';
    ...
    const accessParts = [acessorios, patrimony ? `Patrimônio: ${patrimony}` : null].filter(Boolean);
    const accessories = accessParts.join(' | ') || null;

    const defect     = col.defect       ? String(row[col.defect]      || '').trim() : null;
    const obs        = col.observations ? String(row[col.observations]|| '').trim() : null;

    // Status: PRONTO=S significa concluído, senão usa SITUACAO
    const pronto = col.pronto ? String(row[col.pronto] || '').trim().toUpperCase() : '';
    const status = pronto === 'S' ? 'COMPLETED' : mapStatus(col.status ? row[col.status] : null);
